...
valid_token_summary=np.load("./npdata/"+VALID_FILE +"/token_summary.npy")
valid_token_target=np.load("./npdata/"+VALID_FILE +"/token_target.npy")
...
ckpt_manager=model_saver(bart,bart_optimizer,filename="NOGAN_PRET_LARGE")
# GAN / NOGAN
# PRET / NOPRET
# LARGE / TINY
import datetime
...
import csv
tensorboard_count=0
f= open(consts.FILE_NAME+'/third_results.csv', 'w', newline='')
wr=csv.writer(f)
wr.writerow(['orig_article','summary','ariticle_t','article_g'])
f2= open(consts.FILE_NAME+'/third_loss.csv', 'w', newline='')
wr2=csv.writer(f2)
